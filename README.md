...
The first build downloads the TinyStories model weights (~60 MB total) from HuggingFace into `models/`. Windows/Linux: adjust the compiler/linker in `kernel/GNUmakefile` and use any cross x86_64-elf toolchain.

## Architecture notes

- Boots via [Limine](https://github.com/limine-bootloader/limine); kernel + initrd + model weights are Limine modules on the ISO.
- The kernel is compiled soft-float (`-mno-sse`) so interrupt handlers never touch FP state; only `llm.c`/`llm_math.c` are built hard-float with SSE. No float crosses the API boundary (temperatures are passed in centi-units), and the context switcher `fxsave`s per thread.
